...
class CSR(PublicKey):
    @classmethod
    def generate(cls, private_key, alt_host, name_dict):
        subject = make_x509_name(**name_dict)

        # Generate alternative DNS names
        hosts = []
        for host in alt_host:
        ...
        csr = CSR()
        csr.key = builder.sign(private_key.key,
                               hashes.SHA256(),
                               default_backend())
    ...
    def load(cls, filename):
        ...
        with open(filenamem, "rb") as keyfile:
            data = keyfile.read()
            public_key.key = x509.load_pem_x509_csr(data,
                                                    default_backend())
